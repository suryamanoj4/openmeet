<script lang="ts">
	...
	async function handleSubmit(e: Event) {
		...
		try {
			await updateOrganization(oid, { name, slug, description });
			goto(`/organizations/${oid}`);
		} catch (err) {
...
<div class="mx-auto max-w-2xl px-6 py-8">
	<button onclick={() => goto(`/organizations/${oid}`)} class="flex items-center gap-2 text-body-md text-on-surface-variant hover:text-fg mb-6 transition-colors">
	...
	<Card>
		...
		<CardContent>
			{#if loading}
				<div class="h-48 animate-pulse rounded-lg bg-surface-container"></div>
			{:else}
				<form onsubmit={handleSubmit} class="space-y-5">
					...
					<div class="space-y-1.5"><Label for="desc">Description</Label><textarea id="desc" bind:value={description} class="flex min-h-[100px] w-full rounded-lg border border-input bg-surface-container-lowest px-3 py-2 text-body-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"></textarea></div>
					<div class="flex gap-3 pt-2">
						<Button type="submit" variant="primary" size="lg" isLoading={saving}>Save</Button>
						<Button type="button" variant="outline" size="lg" onclick={() => goto(`/organizations/${oid}`)}>Cancel</Button>
					</div>
				</form>
			{/if}
		</CardContent>
