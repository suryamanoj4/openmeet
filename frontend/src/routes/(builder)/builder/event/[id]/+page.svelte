<script lang="ts">
	...
	type Tab = 'blocks' | 'tickets' | 'preview';
...
<div class="mx-auto max-w-6xl px-6 py-8">
	<button onclick={() => goto(`/events/${$page.params.id as string}`)} class="flex items-center gap-2 text-body-md text-on-surface-variant hover:text-fg mb-6 transition-colors"><ArrowLeft size={18} /> Back to event</button>
					...
					{:else}
						{#each blocks as block, i}
							<Card class="p-5">
								...
								<textarea bind:value={block.content} class="w-full min-h-[80px] rounded-lg border border-input bg-surface-container-lowest px-3 py-2 text-body-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" placeholder="Content..."></textarea>
							</Card>
						{/each}
					{/if}
				</div>
